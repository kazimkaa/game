...
const SPAWN_ANIM_TIMEOUT = 3.0  # предохранитель на случай, если animation_finished не придёт
...
# ✅ ИСПРАВЛЕНИЕ: раньше здесь был "await current_anim.animation_finished",
# но если анимация "spawn" зациклена (Loop = On), этот сигнал вообще
# никогда не приходит в Godot 4 — и await зависал навсегда.
# Пока он висел, is_spawning оставался true, а _physics_process
# в самом начале делает "if is_dead or is_spawning: return",
# поэтому персонаж не двигался и не прыгал после спавна.
# Теперь ждём либо сигнал, либо таймаут — что наступит раньше.
func _play_spawn_animation() -> void:
	if not current_anim or not current_anim.sprite_frames.has_animation("spawn"):
		return

	current_anim.play("spawn")

	var timer = get_tree().create_timer(SPAWN_ANIM_TIMEOUT)
	var finished = false

	var on_finished = func():
		finished = true
	current_anim.animation_finished.connect(on_finished, CONNECT_ONE_SHOT)

	while not finished and timer.time_left > 0:
		await get_tree().process_frame

	if current_anim.animation_finished.is_connected(on_finished):
		current_anim.animation_finished.disconnect(on_finished)
